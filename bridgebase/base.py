...
``BaseSession`` encapsulates the full Phase‑1 lifecycle:
    ...
    → fetch credentials → connect native driver → return native client
...
class BaseSession(ABC):
    ...
    use_tls:
        Whether the gateway socket uses TLS (default ``True``).
    gateway_host:
    ...
    def _do_initialize(self) -> Any:
        ...
        # Step 2: resolve gateway (or use provided endpoint for development)
        if self._gateway_host is not None and self._gateway_port is not None:
            endpoint = GatewayEndpoint(host=self._gateway_host, port=self._gateway_port)
            logger.debug("Using direct gateway endpoint (dev mode): %s:%d", endpoint.host, endpoint.port)
        else:
            endpoint = self._resolver.resolve(self._region, self._jwt_token)

        # Step 3: open control socket
        self._gateway_conn = GatewayConnection(endpoint, self._jwt_token, use_tls=self._use_tls)
